...
class Cl_User
{
	...
	public function registration( array $data )
	{
		if( !empty( $data ) ){
			...
			$query = "INSERT INTO admin (user_id, name, email, password,hash,number,country) VALUES (NULL, '$name', '$email', '$password','$hash','$number', '$country')";
			...
			$to      = $email; // Send email to our user
						$subject = 'Welcome to matchora!'; // Give the email a subject 
...
$headers = 'From:noreply@matchora.com' . "\r\n"; // Set from headers
...
mail($to, $subject, $message, $headers); // Send our email
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "SELECT user_id, name, email FROM admin where email = '$email' and password = '$password' "; // and active='1'
	...
	public function logout()
	{
		session_unset();
		session_destroy();
		header('Location: https://matchora.com/power/');
	}
	...
	public function forgetPassword( array $data )
	{
		if( !empty( $data ) ){
			...
			if(mysqli_query($this->_con, $query)){
				...
				$headers = "From: noreply@matchora.com" . "\r\n" .
						"CC: passwordrequest@matchora.com";
