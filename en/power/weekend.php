...
<?php
   $dat = date_create ("now");
 date_add ($dat, date_interval_create_from_date_string("5 days"));
	...
	if ($mysqli->connect_error){
		die('Error : ('. $mysqli->connect_errno .') '. $mysqli->connect_error);
			 ...
			 if(isset ($_GET['id'])){
				...
				$result = $mysqli->query ("UPDATE users SET accounttwenty ='2', expirytwenty ='$update', LPaymentDate='$date' WHERE user_id ='$id'");
...
$mail->Subject = "WEEKEND 20 Plan Upgrade";
		...
		<tr>
			<td>
				...
				<div style=\"background:#fff;color:#5b5b5b;font-family:'Open Sans', sans-serif;font-size:13px;padding:10px 20px;margin:20px auto;line-height:17px;border:1px #ddd solid;border-top:0;clear:both;margin-top: 0;border-radius: 0 0 10px 10px;\">
					...
					<p>You now have access to the WEEKEND 20 ODDS category on the site</p>
...
$mail->AltBody = "Your account has been upgraded to the WEEKEND 20 ODDS plan";
