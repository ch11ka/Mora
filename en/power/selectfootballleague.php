    ...
    table{
        font-size: 12px;
            ...
            <div class="row">
			   ...
			   <center><form method="post" action="selectfootballleague.php?go">
Enter a date <br><br>

<input type="date" name="mdate"><br><br>
...
if(isset($_POST['submit'])){ 
	...
	$result = $mysqli ->query("SELECT match_id, score, league, game, time, betcode, odds_home, odds_draw, odds_away,fulltimeprediction, result FROM `match` WHERE date = '$mdate' ORDER BY date DESC");
    ...
    <th>Delete</th>
		...
		while ($row = $result->fetch_assoc()){
			...
			echo "<td>" . "<a href=\" ../../../matches/matches.php?id=$row[match_id]\">$row[game]</a>" . "</td>";
			echo "<td>". $row['fulltimeprediction'] ."</td>";			
			if ($row['result'] =="GOOD")
			echo "<td style='background-color:#00ff10;'>". $row['score']."</td>";
		else if ($row['result'] =="BAD")
			echo "<td style='background-color:#FF0000;'>". $row['score']."</td>";
            echo "<td>" . "<a class=\"btn btn-success\" href = \"edit_form.php?id=$row[match_id]\" role=\"button\">Edit</a>"."&nbsp;"."&nbsp;"."</td>";
            
            echo "<td>" ."<a class=\"btn btn-danger\" href = \"delete.php?id=$row[match_id]\" role=\"button\">Delete</a>"."</td>";
