...
.tableholder{
 ...
 @media screen and (max-width: 800px) {
	...
	$result = $mysqli->query ("SELECT user_id,account,expiry, name, email FROM `users` WHERE (user_id = '$name' OR email LIKE '%" . $name ."%')");	
	$row= mysqli_num_rows($result);
	
	if ($row==0){
		
		
			echo "<center>". "<span class=\"blink_me\"> No records exist for <span style=\"color:#ff0000\"> $name</span> </span>". "</center>";
	}	
	else {
	...
	while ($row = $result->fetch_assoc()){
       ...
       echo "<p>";
		...
		echo "<td>" . "<a class=\"btn btn-info btn-xs\" href = \"view.php?id=$row[user_id]\" role=\"button\">View</a>"."</td>";
		
		echo "<td>" . "<a class=\"btn btn-danger btn-xs\" href = \"edit_user.php?id=$row[user_id]\" role=\"button\">Edit</a>"."</td>";
