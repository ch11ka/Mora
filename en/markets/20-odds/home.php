...
<?php
	...
	$result = $mysqli->query ("SELECT match_id,score,league, game,time, betcode, odds_home, odds_draw, odds_away, twentyoddsprediction, twentyodds FROM `match` WHERE twentyodds = 'yes' AND date <= '$forward' AND date >='$back3'  ORDER BY date ASC");
	...
	while ($pow = $result->fetch_assoc()){
		...
		echo "<td>" .$pow ['game'] ."</td>";
		echo "<td>" .$pow ['twentyoddsprediction'] ."</td>";
