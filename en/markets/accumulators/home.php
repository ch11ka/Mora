...
<?php
	...
	$result = $mysqli->query ("SELECT match_id,score,league, game,time, betcode, odds_home, odds_draw, odds_away, accumulatorprediction, accumulator FROM `match` WHERE accumulator = 'yes' AND date ='$date'");
	...
	while ($pow = $result->fetch_assoc()){
		...
		echo "<td>" .$pow ['game'] ."</td>";
		echo "<td>" .$pow ['accumulatorprediction'] ."</td>";
